...
void addSum(){
    vector<int> withArr(n + 1); //permutation array with index as number and value as where it is in the array
    for(int i = 0; i < n; i++){ withArr[arr[i]] = i;}; 
    printVector(withArr);
    set<int> present;
    for(int i = n; i > 0; i--){
        auto it = present.insert(withArr[i]).first;
