...
int main(){
    ...
    int n;
    cin >> n;
    int weneedtoreach = n * n / 2;
    if(n * n % 2 == 1){
        weneedtoreach++;
    }
    //cout << weneedtoreach << endl;
    int arr[501][501];
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            cin >> arr[i][j];
            ...
            for(int a = 0; a < 4; a++){
                ...
                if(nx >= 0 && ny >= 0 && nx < n && ny < n){
                    ...
                    if(elevation > elevEnd || (elevation == elevEnd && (i < nx || j < ny))){
                        edges.push_back({start, end, elevation - elevEnd});
                    }
                }
            }
    ...
    for(Edge edge : edges){
        ...
        maxdiff = max(edge.weight, maxdiff);
        if(dsu.size(edge.a.x * n + edge.a.y) >= weneedtoreach){
            break;
        }
    }
    cout << maxdiff << endl;
