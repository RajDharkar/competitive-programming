...
bool reachable(const info& a, const info& b) {
...
int main() {
    ...
    for (ll i = 0; i < g; ++i) {
        cin >> glaze[i].x >> glaze[i].y >> glaze[i].t;
        cT[i] = glaze[i].t;
    }
    sort(glaze.begin(), glaze.end(), cmp);
    sort(cT.begin(), cT.end());
    ll ans = 0;
    FOR(j, 0, n){
        ll a, b, c;
        cin >> a >> b >> c;
        auto it = lower_bound(cT.begin(), cT.end(), c);
        ll inx = it - cT.begin();
        for(ll i = inx - 1; i <= inx; i++){ //so we check the one directly before and the one directly after
            if(i >= 0 && i < g){
                 if((glaze[i].x - a)*(glaze[i].x - a) + (glaze[i].y - b)*(glaze[i].y - b) > (glaze[i].t - c)*(glaze[i].t - c)) {
                    ans++;
                    break;
                }
            }
