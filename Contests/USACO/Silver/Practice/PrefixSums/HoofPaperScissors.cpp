...
int main() {
    setIO("hps");
    int n = readInt(); 
    vector<int> arr = readVector(n); //paper = 0 //hoof = 1 //scissors = 2
    vector<vector<int>> prefix(n+1, vector<int>(3, 0)); // Initialize with size and values
    ...
    for(int i = 0; i < n; i++){
        ...
        int numSecondPart = 0;
        for(int j = 0; j < 3; j++){
            numFirstPart = max(prefix[i][j], numFirstPart);
            numSecondPart = max(prefix[n][j] - prefix[i][j], numSecondPart);
        }
        maxGamesWon = max(numFirstPart + numSecondPart, maxGamesWon);
    }
    cout << maxGamesWon << '\n';
